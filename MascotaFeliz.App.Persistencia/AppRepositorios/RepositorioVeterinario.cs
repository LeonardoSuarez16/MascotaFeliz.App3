...
namespace MascotaFeliz.App.Persistencia
{
    public class RepositorioVeterinario : IRepositorioVeterinario
    {
        ...
        public Veterinario GetVeterinario(int idVeterinario)
        {
            return _appContext
                .Veterinarios
                .FirstOrDefault(d => d.Id == idVeterinario);
        }

        public string GetFullNameVeterinario(int idVeterinario)
        {
            var veterinario = _appContext.Veterinarios.FirstOrDefault(d => d.Id == idVeterinario);
            if(veterinario!=null){
                return veterinario.Nombres+" "+veterinario.Apellidos;
            }
            return "no se encontro el nombre del veterinario";
        }
        ...
        public Veterinario UpdateVeterinario(Veterinario veterinario)
        {
            var veterinarioEncontrado =
                _appContext
                    .Veterinarios
                    .FirstOrDefault(d => d.Id == veterinario.Id);
        ...
        public IEnumerable<Veterinario> GetVeterinariosPorFiltro(string filtro)
        {
            ...
            if (
                veterinarios != null //Si se tienen saludos
            )
            {
                if (
                ...
                {
                    veterinarios =
                        veterinarios.Where(s => s.Nombres.Contains(filtro));
                }
            }
            return veterinarios;
        }
